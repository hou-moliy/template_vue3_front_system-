    ...
    <el-table border :data="tableData">
      ...
      <el-table-column label="通话" align="center" :index="3">
        ...
        <el-table-column prop="recordMinutes" label="录音分钟数(分钟)" align="center">
          <template #default="{ row }">
            <el-input v-model="row.recordMinutes" :disabled="disabled"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="recordFee" label="录音费用(元)" align="center">
          <template #default="{ row }">
            <el-input v-model="row.recordFee" :disabled="disabled"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="nonRecordMinutes" label="非录音分钟数(分钟)" align="center">
          <template #default="{ row }">
            <el-input v-model="row.nonRecordMinutes" :disabled="disabled"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="nonRecordFee" label="非录音费用(元)" align="center">
          <template #default="{ row }">
            <el-input v-model="row.nonRecordFee" :disabled="disabled"></el-input>
          </template>
        </el-table-column>
      ...
      <el-table-column prop="monthlyTotalFee" label="当月总费用(元)" align="center" :index="4">
        <template #default="{ row }">
          <el-input v-model="row.monthlyTotalFee" :disabled="disabled"></el-input>
        </template>
      </el-table-column>
    </el-table>
    <template #footer>
      ...
      <el-button @click="onSave" v-else :loading="loading">保存</el-button>
    </template>
  </el-dialog>
</template>
