<template>
  ...
  <el-form :inline="true" :model="form" ref="formRef">
    ...
    <el-form-item>
      ...
      <el-button type="primary" v-hasPermi="['phoneOrder:add']" @click="handleAdd">新建任务</el-button>
    </el-form-item>
  </el-form>
  <!-- 表格 -->
  <el-table border :data="tableData" v-load="isLoading">
    ...
    <el-table-column label="详情">
      <template #default="{ row }">
        <el-button type="primary" v-if="row.taskStatus == '2'" link v-hasPermi="['phoneOrder:success']" @click="handleResult(row, 'success')">查看成功</el-button>
        <el-button type="primary" v-if="row.taskStatus == '2'" link v-hasPermi="['phoneOrder:fail']" @click="handleResult(row, 'fail')">查看失败</el-button>
      </template>
    </el-table-column>
  </el-table>
...
const handleResult = ({ taskId }, type) => {
  taskResultRef.value?.openDialog({ taskId, type });
};
onMounted(() => {
  getList();
});
